...
export function titleCase(s) {
  return String(s)
    .toLowerCase()
    .replace(/\b([a-z])([a-z0-9']*)/g, (_, a, b) => a.toUpperCase() + b);
}
...
// Heuristic: does this normalized street name look like junk from raw
// Socrata `violation_street` values? The dataset includes intersection
// strings ("POTOMAC @ ASHLAND"), typos ("POMOTMAC"), and concatenated
// address+street noise ("53052ASHLAND") which all became their own keys.
// We hide them from the search dropdown and leaderboard. Underlying counts
// are unaffected — fix at the source by re-running scripts/build-data.mjs
// with a cleaner filter.
export function isJunkStreet(norm) {
  if (!norm) return true;
  const s = String(norm);
  if (s.includes("@")) return true;
  if (s.includes("/")) return true;
  if (/^[^A-Z]/.test(s)) return true; // doesn't start with a letter
  if (/\d{3,}[A-Z]/.test(s)) return true; // "53052ASHLAND"
  if (/[A-Z]\d{3,}/.test(s)) return true; // "ASHLAND53052"
  return false;
}
...
const RAMP_STOPS = [
  [0.0, [242, 240, 236]], // --surface
  [0.2, [217, 232, 220]],
  [0.45, [121, 173, 132]],
  [0.7, [26, 107, 74]], // --accent
  [1.0, [10, 61, 41]], // --accent-deep
];
